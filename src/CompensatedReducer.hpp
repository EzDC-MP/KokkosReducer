...
// Error Free Transform reducer using TwoSum Summation for Kokkos
#include <Kokkos_Core.hpp>

//!DEPRECATED
template <typename ScalarType, typename AccumulatorType, typename Space>
struct CompensatedReducer
{
  public:
    // Required
    typedef CompensatedReducer<ScalarType, AccumulatorType, Space> reducer;
  ...
  public:
    ...
    //Constructors
    KOKKOS_INLINE_FUNCTION
    CompensatedReducer(value_type& result_) : result(result_) {};
...
struct Scalarhilo
{
  ...
  //operators (add)
    KOKKOS_INLINE_FUNCTION
    Scalarhilo& operator+=(Scalarhilo x) //add assignement using 2Sum algorithm 
    {
      ScalarType sum;
      ScalarType err;
      ScalarType tmp;
